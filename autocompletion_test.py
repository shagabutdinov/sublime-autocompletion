...
test''', 9))
  ...
  def test_get_completion_list_by_word(self):
    ...
    exp = '(?:[^_\w\d]|^)(__WORD__[\w\d]+)'

    self.assertEqual([ 'test' ], method('test', 'te', exp))
    self.assertEqual([ 'test1', 'test2' ], method('test1 test2', 'te', exp))

    actual = method('test1 test2 nope', 'te', exp)
    self.assertEqual([ 'test1', 'test2' ], actual)

    actual = method('test1 test2 test1 nope', 'te', exp)
    self.assertEqual([ 'test1', 'test2' ], actual)

    actual = method('test test1 test2', 'test', exp)
    self.assertEqual([ 'test1', 'test2' ], actual)

  def test_get_text_prepared_for_completion_extraction(self):
    method = self.obj._get_joined_text
  ...
  def test_get_completion_list(self):
    ...
    actual = method([ 'test1', 'test21 test22', 'te test3' ], 2, 2, exp, False)
    self.assertEqual([ 'test3' ], actual)

    actual = method([ 'test1', 'test21 test22', 'te test3 test4' ], 2, 2, exp, False)
    self.assertEqual([ 'test3', 'test4' ], actual)

    actual = method([ 'test1', 'te test21 test22', 'test3' ], 1, 2, exp, False)
    self.assertEqual([ 'test21', 'test22', 'test3' ], actual)

    actual = method([ 'test1 te', 'test21 test22', 'test3' ], 0, 7, exp, True)
    self.assertEqual([ 'test1' ], actual)

    actual = method([ 'test1', 'test21 te test22', 'test3' ], 1, 9, exp, True)
    self.assertEqual([ 'test1', 'test21' ], actual)

    actual = method([ 'te1 test21 test22 test3 te1 test21 te' ], 0, 42, exp, True)
    self.assertEqual([ 'test22', 'test3', 'te1', 'test21' ], actual)

    self.assertEqual(None, method([ 'test1 ' ], 0, 6, exp, True))
  ...
  def test_get_completion_state(self):
    ...
    exp = '(?:[^_\w\d]|^)(__WORD__[\w\d]+)'
    actual = method([ 'test1', 'test21 test22', 'test test3' ], 2, 2, exp, False)
  ...
  def test_get_completion(self):
    ...
    actual = method([ 'test1', 'test2', 'te test3' ], 2, 2, exp)
    self.assertTrue('state' in actual)
    self.assertEqual(actual['completion'], 'test3')

    actual = method([ 'te test3 test4 test5' ], 0, 2, exp)
    self.assertEqual('test3', actual['completion'])

    actual = method([ 'test3 test3 test4 test5' ], 0, 5, exp, False, actual['state'])
    self.assertEqual('test4', actual['completion'])

    actual = method([ 'test4 test3 test4 test5' ], 0, 5, exp, False, actual['state'])
    self.assertEqual('test5', actual['completion'])

    actual = method([ 'test5 test3 test4 test5' ], 0, 5, exp, False, actual['state'])
    self.assertEqual('test5', actual['completion'])

    actual = method([ 'test5 test3 test4 test5' ], 0, 5, exp, True, actual['state'])
    self.assertEqual('test4', actual['completion'])

    actual = method([ 'test4 test3 test4 test5' ], 0, 5, exp, True, actual['state'])
    self.assertEqual('test3', actual['completion'])

    actual = method([ 'test3 test3 test4 test5' ], 0, 5, exp, True, actual['state'])
    self.assertEqual('te', actual['completion'])

    actual = method([ 'te test3 test4 test5' ], 0, 5, exp, True, actual['state'])
    self.assertEqual('te', actual['completion'])
  ...
  def test_get_completion__with_several_lists(self):
    ...
    actual = method([ 'test1 test2', 'test21 test21 test3', 'test4 test5' ], 1,
      13, exp, False, actual['state'])
    self.assertEqual('te', actual['completion'])

    actual = method([ 'test1 test2', 'test21 te test3', 'test4 test5' ], 1, 9,
      exp, False, actual['state'])
    self.assertEqual('test3', actual['completion'])
  ...
  def test_get_completion__dropped_when_cursor_moved(self):
    ...
    actual = method([ 'test1 test2', 'test3 test4 no nope1 nope2 test4',
      'test5 test6' ], 1, 14, exp, False, actual['state'])
  ...
  def test_get_completion__works_at_any_part_of_word(self):
    ...
    actual = method([ 'test1' ], 0, 2, exp)
    self.assertEqual('test1', actual['completion'])

    actual = method([ 'test1', 'test2', 'test3', 'test4' ], 0, 2, exp)
    self.assertEqual('test2', actual['completion'])

    actual = method([ 'test2', 'test2', 'test3', 'test4' ], 0, 5, exp, False,
      actual['state'])
    self.assertEqual('test3', actual['completion'])

    actual = method([ 'test3', 'test2', 'test3', 'test4' ], 0, 5, exp, True,
      actual['state'])
    self.assertEqual('test2', actual['completion'])

    actual = method([ 'test2', 'test2', 'test3', 'test4' ], 0, 5, exp, True,
      actual['state'])
    self.assertEqual('test1', actual['completion'])

    actual = method([ 'test1', 'test2', 'test3', 'test4' ], 0, 5, exp, True,
      actual['state'])
    self.assertEqual('test1', actual['completion'])
  ...
  def test_get_completion__ranges_is_correct(self):
    ...
    actual = method([ 'test1 test2 test test3 test4' ], 0, 14, exp)
    self.assertEqual('test3', actual['completion'])
    self.assertEqual(12, actual['change_start_position'])
    self.assertEqual(16, actual['change_end_position'])

    actual = method([ 'test1 test2 test3 test3 test4' ], 0, 17, exp, False,
      actual['state'])
    self.assertEqual('test4', actual['completion'])
    self.assertEqual(12, actual['change_start_position'])
