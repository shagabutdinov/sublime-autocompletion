...
class AnywordCompletion():

  def get_completion( self, texts, current_text_index, current_cursor_position, direction_desc = False, state = None ):
    word = self._get_word_for_completion( texts[ current_text_index ], current_cursor_position )

    if state is None or 'completion_last' not in state or state['completion_last'] != word:
      state = self._get_completion_state( texts, current_text_index, current_cursor_position, direction_desc )
    ...
    position_start = self._get_completion_insert_position( texts[ current_text_index ], current_cursor_position )
    word_under_cursor = self._get_word_under_cursor( texts[ current_text_index ], current_cursor_position )
    position_end = None
  ...
  def _get_completion_state( self, texts, current_text_index, current_cursor_position, direction_desc = False ):
    word = self._get_word_for_completion( texts[ current_text_index ], current_cursor_position )
    word_under_cursor = self._get_word_under_cursor( texts[ current_text_index ], current_cursor_position )

    completions_desc = self._get_completion_list( texts, current_text_index, current_cursor_position, True ) or []
    completions_asc = self._get_completion_list( texts, current_text_index, current_cursor_position, False ) or []
  ...
  def _get_completion_list( self, texts, current_text_index, current_cursor_position,
    ...
    return self._get_completion_list_by_word( text_prepared, word, direction_desc )

  def _get_text_prepared_for_completion_extraction( self, texts, current_text_index,
    current_cursor_position, direction_desc = False ):

    if direction_desc:
      cursor_position = len( ' '.join( texts[ : current_text_index + 1 ] ) ) + current_cursor_position - \
        len( texts[ current_text_index ] )
  ...
  def _get_completion_list_by_word( self, text, word, direction_desc = False ):
    words = re.findall( r'(?:[^\w\d]|^)(' + re.escape( word ) + r'[\w\d]+)', text, re.M | re.U )
  ...
  def _get_word_for_completion( self, text, current_cursor_position ):
    start_position = self._get_completion_insert_position( text, current_cursor_position )
  ...
  def _get_word_under_cursor( self, text, current_cursor_position ):
    ...
    end_position = re_result = re.search( r'\A([\w\d_]+)', text[ current_cursor_position : ], re.M | re.U )
  ...
  def _get_completion_insert_position( self, text, current_cursor_position ):
    re_result = re.search( r'([\w\d_]+)\Z', text[ : current_cursor_position ], re.M | re.U )
